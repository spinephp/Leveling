...
public class PeilianTixianActivity extends AppCompatActivity implements View.OnClickListener, PayPwdView.InputCallBack  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        img_back_from_tixian = (LinearLayout) findViewById(R.id.img_back_from_peiliantixian);
        input_money = (EditText) findViewById(R.id.input_peilian_money);
        // money_biggest_amount = (TextView) findViewById(R.id.money_biggest_amount);
        input_money_button = (Button) findViewById(R.id.input_peilian_money_button);
    ...
    public void onClick(View v) {
        int id = v.getId();
        switch (id){
            case R.id.input_peilian_money_button:
                if (input_money.getText().toString().equals("")){
                    Toast.makeText(PeilianTixianActivity.this,"请输入提现金额",Toast.LENGTH_SHORT).show();
                }else{
                    if (TixianPeilianActivity.str=="1"){
                        Bundle bundle = new Bundle();
                        bundle.putString(PayFragment.EXTRA_CONTENT, "支付：¥ " + input_money.getText().toString().trim());
                        PayFragment fragment = new PayFragment();
                        fragment.setArguments(bundle);
                        fragment.setPaySuccessCallBack(PeilianTixianActivity.this);
    ...
    public void onInputFinish(String result) {
        ...
        if(TixianPeilianActivity.str=="1"){
            String url = "/api/Pay/GetCashPledge2Withdrawal?money="+input_money.getText().toString().trim()+"&goal="+3+"&paypw="+result1;
            HttpGetUtils.httpGetFile(13,url,handler);
        }else if(TixianPeilianActivity.str=="2"){
            String url = "/api/Pay/GetCashPledge2Withdrawal?money="+input_money.getText().toString().trim()+"&goal="+2+"&paypw="+result1;
            ...
            switch (msg.what){
                case 12:
                    try {
                        ...
                        if (success == "true") {
                            Intent intent  = new Intent(PeilianTixianActivity.this,TixianSuccessActivity.class);
                            startActivity(intent);
                            finish();
                            //Toast.makeText(GetMoneyActivity.this, success, Toast.LENGTH_LONG).show();
                        } else if (success == "false") {
                            Toast.makeText(PeilianTixianActivity.this, err, Toast.LENGTH_LONG).show();
                        }
